...
// Note: https://stackoverflow.com/questions/25468583/why-cant-scala-find-my-typeclass-instance-defined-implicitly-in-the-companion-o
object Math {
...
object Part12 {
  def main(args: Array[String]): Unit = {
    ...
    import Math.NumberImplicits._

    println(s"Mean2: ${Statistics.mean2(xs)}")
    ...
    import org.joda.time.Duration
    import Math.JodaImplicits._

    val ds = (1 to 10).map(Duration.standardSeconds(_)).toVector

    println(s"Mean3: ${Statistics.mean2(ds)}")
    println(s"Median3: ${Statistics.median2(ds)}")
    println(s"Quartiles3: ${Statistics.quartiles2(ds)}")
    println(s"Iqr3: ${Statistics.iqr2(ds)}")
  }
}
